...
def train(config):
  ...
  for iter_num in range(100 + 1):  
    ...
    source_label_np = source_label.cpu().numpy()
    label_temp = torch.zeros(len(source_label_np),2)
    label_temp = label_temp.long()
    for i in range(len(source_label_np)):
      label_temp[i][ source_label_np[i]] = 1
    
    source_label = label_temp.cuda()
  ...
  for iter_num in range(4000 + 1):
    ...
    if (iter_num != 0 and (iter_num + 1) % (iter_per_epoch) == 0):
      ...
      save_checkpoint(save_list, is_best, net2,
                      config.tgt_data + '_baseline.pth.tar')
