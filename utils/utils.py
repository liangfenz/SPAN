...
def accuracy(output, target, topk=(1,)):
  with torch.no_grad():
    ...
    pred_temp = torch.empty(pred.cpu().numpy().shape[0],pred.cpu().numpy().shape[1])
    for i in range(pred.cpu().numpy().shape[0]):
      for j in range(pred.cpu().numpy().shape[1]):
        pred_temp[i,j]=pred[i,j,0]
    pred_temp = pred_temp.t().cuda()    
    target_new = target[:,1]
    correct = pred_temp.eq(target_new.view(1, -1).expand_as(pred_temp))
...
def accuracy_net2(output, target, topk=(1,)):
  with torch.no_grad():
    maxk = max(topk)
    batch_size = target.size(0) 
